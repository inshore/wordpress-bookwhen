...
{
    private const ALLOWED_FILTERS = [FILTER_VALIDATE_BOOLEAN => 'is_bool', FILTER_VALIDATE_DOMAIN => 'is_string', FILTER_VALIDATE_EMAIL => 'is_string', FILTER_VALIDATE_FLOAT => 'is_float', FILTER_VALIDATE_INT => 'is_int', FILTER_VALIDATE_IP => 'is_string', FILTER_VALIDATE_REGEXP => 'is_string', FILTER_VALIDATE_URL => 'is_string'];
    /**
    ...
    public function __construct(int $filter, $options = null)
    {
        ...
        parent::__construct(new Callback(static function ($input) use($filter, $arguments) {
            return self::ALLOWED_FILTERS[$filter](filter_var($input, ...$arguments));
        }));
    }
}
