...
use _PhpScoper6af4d594edb1\phpDocumentor\Reflection\Type;
use _PhpScoper6af4d594edb1\PhpParser\Node;
use _PhpScoper6af4d594edb1\PhpParser\Node\AttributeGroup;
...
abstract class BasePHPElement
{
     ...
     * @return static
     */
    public abstract function readObjectFromReflection($reflectionObject);
    /**
     * @param Node $node
     * @return static
     */
     ...
     */
    public abstract function readMutedProblems($jsonData);
    /**
     * @param Node $node
     * @return string
     */
            ...
            foreach ($attrGroup->attrs as $attr) {
                if ($attr->name->toString() === LanguageLevelTypeAware::class) {
                    ...
                    foreach ($versionTypesMap as $item) {
                        $types[\number_format((float) $item->key->value, 1)] = \explode('|', \preg_replace('/\\w+\\[]/', 'array', $item->value->value));
                    }
                    $maxVersion = \max(\array_keys($types));
                    foreach (new PhpVersions() as $version) {
                        if ($version > (float) $maxVersion) {
                            $types[\number_format($version, 1)] = $types[$maxVersion];
                        }
                    }
