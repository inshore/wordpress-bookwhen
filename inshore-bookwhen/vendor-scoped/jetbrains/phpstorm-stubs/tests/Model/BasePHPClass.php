...
abstract class BasePHPClass extends BasePHPElement
{
    ...
    public function addConstant(PHPConst $parsedConstant)
    {
        if (isset($parsedConstant->name)) {
            if (array_key_exists($parsedConstant->name, $this->constants)) {
                $amount = count(\array_filter($this->constants, function (PHPConst $nextConstant) use($parsedConstant) {
    ...
    public function getConstant($constantName)
    {
        $constants = \array_filter($this->constants, function (PHPConst $constant) use($constantName) {
            return $constant->name === $constantName && $constant->duplicateOtherElement === \false && BasePHPElement::entitySuitsCurrentPhpVersion($constant);
        });
        if (empty($constants)) {
    ...
    public function getMethod($methodName)
    {
        $methods = \array_filter($this->methods, function (PHPMethod $method) use($methodName) {
            return $method->name === $methodName && $method->duplicateOtherElement === \false && BasePHPElement::entitySuitsCurrentPhpVersion($method);
        });
        if (empty($methods)) {
