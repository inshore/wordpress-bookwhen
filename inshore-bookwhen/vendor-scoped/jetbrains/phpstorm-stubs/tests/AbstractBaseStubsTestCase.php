...
use _PhpScoper6af4d594edb1\PhpParser\Node\Expr\Array_;
use _PhpScoper6af4d594edb1\PhpParser\Node\Expr\BinaryOp\BitwiseOr;
use _PhpScoper6af4d594edb1\PhpParser\Node\Expr\ClassConstFetch;
use _PhpScoper6af4d594edb1\PhpParser\Node\Expr\ConstFetch;
use _PhpScoper6af4d594edb1\PhpParser\Node\Expr\UnaryMinus;
use _PhpScoper6af4d594edb1\PhpParser\Node\Scalar\DNumber;
use _PhpScoper6af4d594edb1\PhpParser\Node\Scalar\LNumber;
    ...
    public static function getStringRepresentationOfDefaultParameterValue(mixed $defaultValue, PHPClass|PHPInterface $contextClass = null) : float|bool|int|string|null
    {
        if ($defaultValue instanceof ConstFetch) {
            ...
            if ($defaultValue->left instanceof ConstFetch && $defaultValue->right instanceof ConstFetch) {
                $constants = array_filter(PhpStormStubsSingleton::getPhpStormStubs()->getConstants(), fn(PHPConst $const) => property_exists($defaultValue->left, 'name') && $const->name === (string) $defaultValue->left->name);
                /** @var PHPConst $leftConstant */
                $leftConstant = array_pop($constants);
                $constants = array_filter(PhpStormStubsSingleton::getPhpStormStubs()->getConstants(), fn(PHPConst $const) => property_exists($defaultValue->right, 'name') && $const->name === (string) $defaultValue->right->name);
