...
use _PhpScoper6af4d594edb1\StubTests\Model\BasePHPElement;
...
class BaseFunctionsTest extends AbstractBaseStubsTestCase
{
    ...
    public function testFunctionsDeprecation(PHPFunction $function)
    {
        $functionName = $function->name;
        $stubFunction = PhpStormStubsSingleton::getPhpStormStubs()->getFunction($functionName);
        static::assertFalse($function->isDeprecated && $stubFunction->isDeprecated !== \true, "Function {$functionName} is not deprecated in stubs");
    }
    /**
    ...
    public function testFunctionsOptionalParameters(PHPFunction $function, PHPParameter $parameter)
    {
        ...
        self::assertEquals($parameter->isOptional, $stubOptionalParameter->isOptional, \sprintf('Reflection function %s %s optional parameter %s with index %d 
            but stubs parameter %s with index %d %s', $function->name, $parameter->isOptional ? 'has' : 'has no', $parameter->name, $parameter->indexInSignature, $stubOptionalParameter->name, $stubOptionalParameter->indexInSignature, $stubOptionalParameter->isOptional ? 'is optional' : 'is not optional'));
        self::assertEquals($parameter->is_vararg, $stubOptionalParameter->is_vararg, \sprintf('Reflection function %s %s vararg parameter %s with index %d 
    ...
    public function testMethodsOptionalParameters(PHPClass|PHPInterface $class, PHPMethod $method, PHPParameter $parameter)
    {
        ...
        /** @var PHPParameter $stubOptionalParameter */
        $stubOptionalParameter = \array_pop($stubParameters);
        self::assertEquals($parameter->isOptional, $stubOptionalParameter->isOptional, \sprintf('Reflection method %s::%s has %s optional parameter %s with index %d but stub parameter %s with index %d is %s optional', $class->name, $method->name, $parameter->isOptional ? "" : "not", $parameter->name, $parameter->indexInSignature, $stubOptionalParameter->name, $stubOptionalParameter->indexInSignature, $stubOptionalParameter->isOptional ? "" : "not"));
    }
    /**
     * @throws Exception
     */
    ...
    {
        $implodeFunctions = \array_filter(PhpStormStubsSingleton::getPhpStormStubs()->getFunctions(), fn(PHPFunction $function) => $function->name === 'implode');
