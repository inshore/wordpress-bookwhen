...
use _PhpScoper6af4d594edb1\StubTests\Model\PhpVersions;
...
class BaseClassesTest extends AbstractBaseStubsTestCase
{
    ...
    public function testClassesParent(PHPClass|PHPInterface $class)
    {
        ...
        if ($class instanceof PHPClass) {
            ...
            static::assertEquals($class->parentClass, $stubClass->parentClass, empty($class->parentClass) ? "Class {$className} should not extend {$stubClass->parentClass}" : "Class {$className} should extend {$class->parentClass}");
        } else {
            ...
            foreach ($class->parentInterfaces as $parentInterface) {
                static::assertContains($parentInterface, $stubClass->parentInterfaces, "Interface {$className} should extend {$parentInterface}");
            }
        }
    }
    /**
    ...
    public function testClassesFinalMethods(PHPClass|PHPInterface $class, PHPMethod $method)
    {
        ...
        static::assertEquals($method->isFinal, $stubMethod->isFinal, "Method {$className}::{$method->name} final modifier is incorrect");
    }
    /**
    ...
    public function testClassesStaticMethods(PHPClass|PHPInterface $class, PHPMethod $method)
    {
        ...
        static::assertEquals($method->isStatic, $stubMethod->isStatic, "Method {$className}::{$method->name} static modifier is incorrect");
    }
    /**
    ...
    public function testClassesMethodsVisibility(PHPClass|PHPInterface $class, PHPMethod $method)
    {
        ...
        static::assertEquals($method->access, $stubMethod->access, "Method {$className}::{$method->name} access modifier is incorrect");
    }
    /**
    ...
    public function testClassMethodsParametersCount(PHPClass|PHPInterface $class, PHPMethod $method)
    {
        ...
        static::assertSameSize($method->parameters, $filteredStubParameters, "Parameter number mismatch for method {$className}::{$method->name}.\n                         Expected: " . self::getParameterRepresentation($method));
    }
    /**
    ...
    public function testClassInterfaces(PHPClass $class)
    {
        ...
        foreach ($class->interfaces as $interface) {
            static::assertContains($interface, $stubClass->interfaces, "Class {$className} doesn't implement interface {$interface}");
        }
    }
    /**
    ...
    public function testClassProperties(PHPClass $class, PHPProperty $property)
    {
        ...
        static::assertNotEmpty($stubClass->getProperty($property->name), "Missing property {$property->access} " . \implode('|', $property->typesFromSignature) . "{$className}::\${$property->name}");
    }
    /**
    ...
    public function testClassStaticProperties(PHPClass $class, PHPProperty $property)
    {
        ...
        static::assertEquals($property->is_static, $stubProperty->is_static, "Property {$className}::{$property->name} static modifier is incorrect");
    }
    /**
    ...
    public function testClassPropertiesVisibility(PHPClass $class, PHPProperty $property)
    {
        $className = $class->name;
        $stubProperty = PhpStormStubsSingleton::getPhpStormStubs()->getClass($class->name)->getProperty($property->name);
        static::assertEquals($property->access, $stubProperty->access, "Property {$className}::{$property->name} access modifier is incorrect");
    }
    /**
    ...
    public function testClassesReadonly(PHPClass $class) : void
    {
        $className = $class->name;
        $stubClass = PhpStormStubsSingleton::getPhpStormStubs()->getClass($className);
        static::assertEquals($class->isReadonly, $stubClass->isReadonly, "Readonly modifier for class {$className} is incorrect");
    }
}
