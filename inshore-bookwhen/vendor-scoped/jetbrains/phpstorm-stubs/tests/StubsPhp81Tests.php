...
class StubsPhp81Tests extends AbstractBaseStubsTestCase
{
    ...
    public function testPropertyReadonly(PHPClass $class, PHPProperty $property)
    {
        ...
        static::assertEquals($property->isReadonly, $stubProperty->isReadonly, "Property {$className}::{$property->name} readonly modifier is incorrect");
    }
    /**
    ...
    public function testTentativeReturnTypeHints(PHPClass|PHPInterface $class, PHPMethod $method)
    {
        ...
        self::assertTrue($testCondition, "Method {$class->name}::{$functionName} has invalid return type. Reflection method has return type " . \implode('|', $method->returnTypesFromSignature) . ' but stubs has return type ' . \implode('|', $stubMethod->returnTypesFromSignature) . ' in signature and attribute has types ' . \implode('|', $typesFromAttribute));
        self::assertTrue($stubMethod->isReturnTypeTentative, "Reflection method {$class->name}::{$functionName} has " . "tentative return type but stub's method isn't declared as tentative");
    }
}
