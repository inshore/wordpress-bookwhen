 ...
 */
namespace _PhpScoper6af4d594edb1\Monolog;

use Closure;
use DateTimeZone;
use Fiber;
use _PhpScoper6af4d594edb1\Monolog\Handler\HandlerInterface;
use _PhpScoper6af4d594edb1\Monolog\Processor\ProcessorInterface;
use _PhpScoper6af4d594edb1\Psr\Log\LoggerInterface;
use _PhpScoper6af4d594edb1\Psr\Log\InvalidArgumentException;
...
class Logger implements LoggerInterface, ResettableInterface
{
    ...
    public function addRecord(int|Level $level, string $message, array $context = [], DateTimeImmutable $datetime = null) : bool
    {
        ...
        try {
            ...
            $record = new LogRecord(message: $message, context: $context, level: self::toMonologLevel($level), channel: $this->name, datetime: $datetime ?? new DateTimeImmutable($this->microsecondTimestamps, $this->timezone), extra: []);
            $handled = \false;
    ...
    {
        $record = new LogRecord(datetime: new DateTimeImmutable($this->microsecondTimestamps, $this->timezone), channel: $this->name, message: '', level: self::toMonologLevel($level));
        foreach ($this->handlers as $handler) {
            if ($handler->isHandling($record)) {
                return \true;
            }
