...
class EventTest extends TestCase
{
     ...
     * @covers InShore\Bookwhen\ValueObjects\Transporter\QueryParams
     */
    public function testValidEventIdIncludeLocation() : void
    {
        $this->mockHandler->append(new Response(200, [], \file_get_contents(__DIR__ . '/../fixtures/event_include_location_200.json')));
        $this->client = BookwhenApi::factory()->withApiKey($this->apiKey)->withHttpClient($this->guzzleClient)->make();
        $bookwhen = new Bookwhen(null, $this->client);
        $event = $bookwhen->event('ev-s4bs-20230501080000');
        ...
        $this->assertEquals('Online', $event->location->additionalInfo);
        $this->assertEquals('Zoom', $event->location->addressText);
        $this->assertEquals(49.21879, $event->location->latitude);
        $this->assertEquals(-2.12625, $event->location->longitude);
        $this->assertEquals('w0uh48ad3fm2', $event->location->id);
        $this->assertEquals('w0uh48ad3fm2', $event->location->id);
        $this->assertEquals(10, $event->maxTicketsPerBooking);
        $this->assertEquals('2023-05-01T08:00:00.000Z', $event->startAt);
