...
use _PhpScoper6af4d594edb1\Safe\Exceptions\SwooleException;
/**
 *
 *
...
{
    \error_clear_last();
    $safeResult = \_PhpScoper6af4d594edb1\swoole_async_dns_lookup($hostname, $callback);
    if ($safeResult === \false) {
...
{
    \error_clear_last();
    $safeResult = \_PhpScoper6af4d594edb1\swoole_async_readfile($filename, $callback);
    if ($safeResult === \false) {
        throw SwooleException::createFromPhpError();
    }
}
...
function swoole_async_write(string $filename, string $content, int $offset = null, callable $callback = null) : void
{
    \error_clear_last();
    if ($callback !== null) {
        $safeResult = \_PhpScoper6af4d594edb1\swoole_async_write($filename, $content, $offset, $callback);
    } elseif ($offset !== null) {
        $safeResult = \_PhpScoper6af4d594edb1\swoole_async_write($filename, $content, $offset);
    } else {
        $safeResult = \_PhpScoper6af4d594edb1\swoole_async_write($filename, $content);
    }
    if ($safeResult === \false) {
        throw SwooleException::createFromPhpError();
    }
...
function swoole_async_writefile(string $filename, string $content, callable $callback = null, int $flags = 0) : void
{
    \error_clear_last();
    if ($flags !== 0) {
        $safeResult = \_PhpScoper6af4d594edb1\swoole_async_writefile($filename, $content, $callback, $flags);
    } elseif ($callback !== null) {
        $safeResult = \_PhpScoper6af4d594edb1\swoole_async_writefile($filename, $content, $callback);
    } else {
        $safeResult = \_PhpScoper6af4d594edb1\swoole_async_writefile($filename, $content);
    }
    if ($safeResult === \false) {
        throw SwooleException::createFromPhpError();
    }
